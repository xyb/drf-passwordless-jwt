...
class VerifyJWTHeaderView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not authorization_header:
            return Response(status=status.HTTP_400_BAD_REQUEST, data={'error': 'Authorization header must be provided'})

        try:
            _, token = authorization_header.split()
        except ValueError:
            return Response(status=status.HTTP_400_BAD_REQUEST, data={'error': 'Invalid Authorization header format'})
